...
from . import ConstantImage, ConstantImageList, Channel, Equals, Grayscale, Invert, Merge, Threshold, Moments, GetImageProperties, ConvertDepth
...
from .filter import AverageBlur, BilateralFilter, GaussianBlur, LinearFilter, MedianBlur, GaussianFunction
...
class TestPipes(unittest.TestCase):    
    ...
    test_image_gray_rgb = np.random.randint(0, 255, (256,256,3), dtype=np.uint8)
    ...
    def test_image_arithmetic_ImageAdd(self):
        ...
        temp = test_image.astype(dtype=np.uint16, copy=True)
        temp = temp + temp
        temp[temp>255] = 255
    ...
    def test_image_filter_GaussianBlur(self):

        test_image = np.random.randint(0, 255, (256,256), dtype=np.uint8)

        with GraphBuilder() as graph:
            n0 = ConstantImage(source=test_image)
            o0 = GaussianBlur(source=n0, size_x=3, size_y=3, sigma_x=0.0, sigma_y=0.0)
            o1 = GaussianBlur(source=n0, size_x=5, size_y=5, sigma_x=0.25, sigma_y=0.25)
            o2 = GaussianBlur(source=n0, size_x=7, size_y=7, sigma_x=0.5, sigma_y=0.5)
            o3 = GaussianBlur(source=n0, size_x=9, size_y=9, sigma_x=0.75, sigma_y=0.75)
    ...
    def test_image_filter_AverageBlur(self):

        test_image = np.random.randint(0, 255, (256,256), dtype=np.uint8)

        with GraphBuilder() as graph:
            n0 = ConstantImage(source=test_image)
            o0 = AverageBlur(source=n0, size=3)
            o1 = AverageBlur(source=n0, size=5)
            o2 = AverageBlur(source=n0, size=7)
            o3 = AverageBlur(source=n0, size=9)          
            Output(outputs=[o0, o1, o2, o3])
    ...
    def test_image_image_ConstantImageList(self):
        ...
        np.testing.assert_array_equal(output[0][0], test_image_list[0][0,:,:])
        np.testing.assert_array_equal(output[0][1], test_image_list[1][0,:,:])
    ...
    def test_image_image_ConvertDepth(self):
        ...
        self.assertIs(output[0].dtype, np.dtype('ubyte'))
        self.assertIs(output[1].dtype, np.dtype('short'))
