...
Image::Image(cv::Mat value) : ScalarVariable<cv::Mat>(value) {
    
}

void Image::describe(std::ostream& os) const {
...
SharedVariable ConstantImage(std::vector<SharedVariable> inputs, ContextHandle context, cv::Mat image) {
 ...
 * 
 */
SharedVariable Moments(std::vector<SharedVariable> inputs, ContextHandle context, bool binary) noexcept(false) {
 ...
 * 
 */
SharedVariable ImageThreshold(std::vector<SharedVariable> inputs, ContextHandle context) noexcept(false) {
